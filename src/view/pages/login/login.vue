<template>
    ...
    <div class="logo">
      <div
        class="login-logo"
        v-if="loginLogo"
        v-bg-image="loginLogo">
      </div>
      <svg v-if="!loginLogo">
        <use xlink:href="#logo_login"></use>
      </svg>
    </div>
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position="left" label-width="0px" class="login-container">
      ...
      <el-form-item prop="account" class="icon-login icon-user">
        <el-input type="text" v-model="user.username" size="large" :maxlength="20" auto-complete="off" placeholder="用户名">
        </el-input>
      </el-form-item>
      <el-form-item prop="checkPass" class="icon-login icon-pass">
        <el-input type="password" v-model="user.password" size="large" :maxlength="32" auto-complete="off" placeholder="登录密码"></el-input>
      </el-form-item>
